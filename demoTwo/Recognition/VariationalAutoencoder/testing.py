...
class VAE(nn.Module):
    def __init__(self, z_dim=2, img_size=128):
        super().__init__()
        ...
        self.enc_out = 256 * self.h * self.h
    ...
    def decode(self, z):
        h = self.fc_dec(z)
        h = h.view(z.size(0), 256, self.h, self.h)
        x = self.dec(h)
        return x
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"Epoch [{epoch}/{args.epochs}] Train: total={run_tot:.4f} rec={run_rec:.4f} kld={run_kld:.4f} | Val: total={val_tot:.4f} rec={val_rec:.4f} kld={val_kld:.4f} cap={cap:.3f}")
